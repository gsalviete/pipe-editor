...
| **IR-AC-020** | **`container.image` is a valid image reference.** `validate()` rejects a `container.image` that does not match `[registry[:port]/]name[:tag][@sha256:<64 hex>]`, lowercase throughout. A value beginning with `-`, containing whitespace, or carrying an uppercase path component fails with `IR-AC-020`. Rationale: the Executor builds `docker run … <image> sh -c <command>`, so a value in the option region is read by the Docker CLI as another option and shifts the image slot. The check lives in `validate()` so the Executor, both CI exporters, the Dockerfile Generator, the Workspace Plan validator and the Editor inherit it rather than each carrying a copy. | T-IR-020 |
| **IR-AC-021** | **`step.env` keys are environment variable names and values are strings.** `validate()` rejects any key not matching `^[A-Za-z_][A-Za-z0-9_]*$` and any non-string value, with `IR-AC-021`. Rationale: env entries flow into `-e KEY=VALUE` on the docker argv, into `env:` in the GitHub Actions export and into `variables:` in GitLab; a key containing `:`, a leading `-`, or a newline yields a different argv or a different document than the one the editor displayed. | T-IR-021 |
| **IR-AC-022** | **Document nesting is bounded.** `validate()` returns an `IR-AC-022` error rather than throwing when a document nests deeper than `MAX_DOCUMENT_DEPTH` (64), and `serializeCanonical` / `canonicalDigest` throw a `RangeError` naming the limit. Rationale: the forbidden-key walk and the canonical key sort both recurse over the whole document; Express accepts 1 MB bodies and V8 parses JSON iteratively, so a deeply-nested body reached the recursive walk and exhausted the stack, surfacing as an opaque 500 instead of a 400. A real IR nests about six levels. | T-IR-022 |
...
| 2026-09-13 | Amendment (stays Accepted), from adversarial review findings **SEC-03**, **SEC-04** and **SEC-07**. `validate()` checked the *shape* of `container.image` and `step.env` (a string; an object) but never their *values*, and both leave the IR for places that reinterpret them: the image lands in the option region of `docker run`, and env keys land in a shell argv and in two YAML documents. Nesting was unbounded on a recursive walk fed by a 1 MB request body. Three new criteria — IR-AC-020 (image reference grammar), IR-AC-021 (env key grammar and string values), IR-AC-022 (depth bound, reported as a validation error rather than a stack overflow). All three are enforced inside `validate()` so every consumer inherits them once (ADR-0003); `canonical.ts` carries the same depth bound because it runs on documents that have not necessarily been validated first. `AcceptanceCriterionId` widened accordingly. Total ACs: 22 (was 19). |
