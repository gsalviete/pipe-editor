...
|---|---|
| Component | `DET` |
| Status | Implemented · amendment **Draft** (Hardening v2 Phase 1, 2026-09-23) — DET-FR-020, DET-AC-022…023 await acceptance |
...
- **DET-FR-020 — Manifests are read through the contained read.**
  *(Draft — Hardening v2 Phase 1, AR-02 / AR-03.)* Step 2 of the Engine
...
| **DET-AC-022** *(Draft)* | `detect()` on a project whose `package.json` is a symlink to a file outside the project directory throws an error citing `package.json` and a containment reason (not a parse diagnostic), and never returns the outside file's content. The same project with only `tsconfig.json` linked outside completes, with `tsconfig.json` treated as absent and a warning naming it. Inverts review probe R02 on the detect path. | T-DET-022 (TASK-003) |
| **DET-AC-023** *(Draft)* | `detect()` on a project whose `package.json` is a FIFO (or a directory) throws a special-file diagnostic within 1 second. A FIFO at `.nvmrc` yields a warning and no blocking. | T-DET-023 (TASK-003) |
