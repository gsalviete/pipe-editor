...
|---|---|
| Component | `DET` |
| Status | Implemented · amendment **Accepted** (Hardening v2 Phase 1, 2026-09-23) — DET-FR-020, DET-AC-022…023 accepted, awaiting implementation |
...
- **DET-FR-019 — `package.json` hard-throw on parse failure.** If
  ...
  in fact broken.
- **DET-FR-020 — Manifests are read through the contained read.**
  *(Accepted — Hardening v2 Phase 1, AR-02 / AR-03.)* Step 2 of the Engine
  pipeline MUST read every manifest in the enumerated set through the
  contained read of
  [STATE-FR-017](./state.spec.md#discovery) /
  [ADR-0019](../adr/0019-filesystem-boundary-reads-and-writes.md), with
  `rootPath` (the project directory) as the boundary. A refused manifest (it
  resolves outside `rootPath`, isn't a regular file, or exceeds the budget)
  is handled by DET-FR-019's split:
  - **`package.json`:** `detect()` throws, citing the path and the reason
    for the refusal (containment, special file or oversized). The refusal
    reason is distinct from a parse diagnostic.
  - **Every other manifest:** treated as absent, with a warning naming the
    manifest and the reason.

  The existence and "is it a file" checks that used to precede the read are
  the contained read's job. A missing manifest is still simply absent.
...
| **DET-AC-022** *(Accepted)* | `detect()` on a project whose `package.json` is a symlink to a file outside the project directory throws an error citing `package.json` and a containment reason (not a parse diagnostic), and never returns the outside file's content. The same project with only `tsconfig.json` linked outside completes, with `tsconfig.json` treated as absent and a warning naming it. Inverts review probe R02 on the detect path. | T-DET-022 (TASK-003) |
| **DET-AC-023** *(Accepted)* | `detect()` on a project whose `package.json` is a FIFO (or a directory) throws a special-file diagnostic within 1 second. A FIFO at `.nvmrc` yields a warning and no blocking. | T-DET-023 (TASK-003) |
...
| 2026-09-23 | **Amendment — Draft** (Hardening v2 Phase 1, `tasks/TASK-001`), from the second adversarial review's **AR-02** / **AR-03**. Step 2 checked that a manifest existed and was a file on its **path**, then read whatever the path named. A `package.json` symlink leaked an outside file's content into the IR (R02), and a FIFO manifest blocked the process. New **DET-FR-020** routes every manifest read through the contained read of STATE-FR-017 / [ADR-0019](../adr/0019-filesystem-boundary-reads-and-writes.md), with `rootPath` as the boundary, and maps refusals onto DET-FR-019's existing split (throw for `package.json`, warn and treat as absent for the rest). New **DET-AC-022…023**. |
| 2026-09-23 | **Amendment accepted** (Hardening v2 Phase 1, `tasks/TASK-001`). The owner accepted DET-FR-020 and DET-AC-022…023, together with ADR-0019's contained-symlink policy, as proposed. The spec stays Implemented for its existing criteria; the new ones are Accepted and become Implemented when their tests pass. |
